...
func TestNeighbourLeftmostedge(t *testing.T) {
	grid := NewGrid(4, 0, 0)
	actual := CountAliveneighbours(1, 0, grid)
	Expected := uint(1)
	if actual != Expected {
		t.Error("Expected", Expected, "bt got", actual)
	}
	...
	grid = NewGrid(4, 0, 0, 0, 1, 1, 1, 2, 1)
	actual = CountAliveneighbours(1, 0, grid)
	Expected = uint(4)
	if actual != Expected {
		t.Error("Expected", Expected, "bt got", actual)
	}
	grid = NewGrid(4, 0, 0, 0, 1, 1, 1, 2, 1, 2, 0)
	actual = CountAliveneighbours(1, 0, grid)
	Expected = uint(5)
	if actual != Expected {
		t.Error("Expected", Expected, "bt got", actual)
...
func TestNeighbourRightmostedge(t *testing.T) {
	grid := NewGrid(4, 0, 3)
	actual := CountAliveneighbours(1, 3, grid)
	Expected := uint(1)
	if actual != Expected {
		t.Error("Expected", Expected, "bt got", actual)
	}
	...
	grid = NewGrid(4, 0, 3, 0, 2, 1, 2, 2, 2)
	actual = CountAliveneighbours(1, 3, grid)
	Expected = uint(4)
	if actual != Expected {
		t.Error("Expected", Expected, "bt got", actual)
	}
	grid = NewGrid(4, 0, 3, 0, 2, 1, 2, 2, 2, 2, 3)
	actual = CountAliveneighbours(1, 3, grid)
	Expected = uint(5)
	if actual != Expected {
		t.Error("Expected", Expected, "bt got", actual)
...
func TestRunGeneration_Reproduction(t *testing.T) {
	...
	if !nextGen.data[1][1] {
		t.Error("Expected middle cell (1,1) to become alive due to reproduction, but it's still dead")
	}
}
